...
import { Loader2, ArrowLeft, ExternalLink, Lightbulb } from "lucide-react";
import { Button } from "@/components/ui/button";
import { apiRequest } from "@/lib/queryClient";
import { Problem, Trick } from "@shared/schema";
import { renderMarkdown } from "@/lib/markdown";
import { useToast } from "@/hooks/use-toast";

export default function TrickDetail() {
  const [, setLocation] = useLocation();
  const [match, params] = useRoute<{ id: string }>("/tricks/:id");
  ...
  const { 
    data: trick, 
    isLoading: isTrickLoading, 
    isError: isTrickError,
    error: trickError 
  } = useQuery<Trick>({
    ...
    enabled: !!trickId,
    onError: (error) => {
      console.error("Trick query error:", error);
      toast({
        title: "Error loading trick",
        description: `${error}`,
        variant: "destructive"
      });
    }
  ...
  const isLoading = isTrickLoading || isProblemsLoading;
  const isError = isTrickError || isProblemsError;
    ...
    return (
      <div className="container mx-auto px-4 py-6 flex justify-center items-center min-h-[50vh]">
        <div className="text-center">
          <h2 className="text-xl font-semibold text-red-500 mb-2">Error Loading Trick</h2>
          <p className="text-slate-600 mb-2">
            Unable to load trick details. Please try again.
          </p>
          <p className="text-sm text-slate-500 mb-4">
            {trickError ? `Error: ${trickError.message}` : 
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mb-6 p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="text-slate-500 mt-1">
              Used in {trick.usageCount || 0} problems
            </div>
          </div>
        </div>
        ...
        <div className="prose max-w-none dark:text-slate-300 mt-4">
          {trick.description ? 
            renderMarkdown(trick.description) : 
        ...
        <div>
          {relatedProblems && relatedProblems.length > 0 ? (
            ...
            <div className="p-6 text-center text-slate-500 dark:text-slate-400">
              <p>No problems are currently using this trick.</p>
            </div>
          )}
        </div>
      </div>
